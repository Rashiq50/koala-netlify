...
export default function ProductEdit() {
    ...
    const addProduct = () => {
        const productIndex = globalState.products.findIndex(el => el.id == product.id);
        const tmp = globalState.products;
        tmp[productIndex] = product;
        setGlobalState({ ...globalState, "products": [...tmp] });
        setFinishModal(true);
    }

    useEffect(()=>{
        const splittedPath = window.location.pathname.split('/');
        const productId = splittedPath[splittedPath.length - 2];
        const productIndex = globalState.products.findIndex(el => el.id == productId);
        setProduct(globalState.products[productIndex]);
    },[])
        ...
        <div className="container mx-auto py-10 px-12">
            ...
            >
                <div className="w-full flex gap-x-96 items-center py-6 px-10">
                    <div className="text-2xl font-semibold text-[#20215A]">Product details updated!</div>
